(ns lidoc-lufthansa.main
  "Kotodama WASM entrypoint for the Lidoc_lufthansa clean-room actor (L4) — Clojure port.
  ...
  facts (`lidoc_lufthansa.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "lidoc_lufthansa")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"lidoc_lufthansa.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "lidoc_lufthansa-compat" :tier tier :entities entities} 200])
...
(defn start! [] :lidoc_lufthansa-compat/ready)
